...
formatted dumps
hex print words .L .B etc
prevent data stack reset on errors
general control keys - ' ^ctrls is ?ctrls  (moved this to EXTEND)
call - fix
reboot - fix
mask
[ctrl] which compiles the value for the ascii control key

TO DO:
-- call jump
restart
words vs vlist - verbose version
system timer - testing in EXTEND
single chars as numbers ^Q 'Q' etc
...
#define TIER0_OPCODE_LIST \
  ...
  Y(CALL, w = tos; DROP; JMPW) \
  Y(REBOOT, setup()) \
  Y(HIGH, pinMode(n0, 2); digitalWrite(n0, 1); DROP) \
  Y(LOW, pinMode(n0, 2); digitalWrite(n0, 0); DROP) \
  Y(INP, pinMode(n0, 1); DROP) \
...
#define TIER1_OPCODE_LIST \
  ...
  X("++", INC1, *((cell_t *) tos) += 1; DROP) \
  X("--", DEC1, *((cell_t *) tos) += -1; DROP) \
  X("~",  CLEAR, *((cell_t *) tos) = 0; DROP) \
  X("~~", UNCLEAR, *((cell_t *) tos) = -1; DROP) \
...
static cell_t convert(const char *pos, cell_t n, cell_t base, cell_t *ret) {
  *ret = 0;
  cell_t negate = 0;
  if (!n) { return 0; }
  if (*pos == '\'' & *(pos+2) == '\'' ) {  *ret = *(pos+1) ; return -1; }
  if (*pos == '^') { *ret = (*(pos+1) & 0x1f); return -1; }
...
#undef Z
  ...
  register cell_t *ip, *rp, *sp, tos, w, index, limit, branch;
...
( Quoting Words ) ( char modes deprecated in favor of 'n' literals )
...
( FOR..NEXT )
create FOR ' >r @ ' for !         : for   nest-depth ++ ['] for , here ; immediate
create NEXT ' donext @ ' next !   : next  nest-depth -- ['] next , , ; immediate

( DO..LOOP )
variable leaving
: leaving,   here leaving @ , leaving ! ;
: leaving(   leaving @ leaving ~   2 nest-depth +! ;
...
: u.r ( u n -- ) <# 1- for # next #> type ;
: print 	<# #s #> type ;
: u. ( u -- )   print space ;
: . ( w -- )  	base @ 10 xor if u. exit then str type space ;
: ? ( a -- )  	@ . ;
: n. ( n -- ) 	base @ swap decimal print base ! ;
...
0 	value _ansi
: ANSI ( n -- )		_ansi 0= IF FOR AFT DROP THEN NEXT R> THEN DROP ;

: CLS 			12 EMIT ;

: ESCX ( ch -- )	27 EMIT EMIT ;
: ESC[ 			[char] [ ESCX ;
: HOME			0 ANSI [char] H ESC[ EMIT ;
: COL ( col fg/bg -- )	ESC[ EMIT [char] 0 + EMIT [char] m EMIT ;
: PEN ( col -- )	1 ANSI 7 AND [char] 3 COL ;
: PAPER ( col -- )	1 ANSI [char] 4 COL ;


( 0 plain 1 bold 2 dim 3 rev 4 uline )
: ATR ( ch -- )		ESC[ EMIT [char] m EMIT ;
: PLAIN			0 ANSI [char] 0 ATR ;

: REVERSE		0 ANSI [char] 7 ATR ;
: BOLD			0 ANSI [char] 1 ATR ;
: UL          		0 ANSI [char] 4 ATR ;
: BLINKING	        0 ANSI [char] 5 ATR ;

: _CSI			SWAP PRINT EMIT ;
...
( Better Errors )
: .ERROR	plain bold red pen ."  *ERROR* " ;
: notfound ( a n n -- )
   if .ERROR type ."  NOT FOUND! "  -1 throw then ;
' notfound 'notfound !
...
: .s   ." <" depth n. ." > " raw.s cr ;
\ .s   depth 0 max for aft cr sp@ r@ cells - @ dup .l space . then next ;

variable echo echo ~~   0 value prevch
...
\ defer ?ctrls ' noop is ?ctrls

: *key ( -- n )
  begin
    key
    dup nl = if drop prevch 13 = if 0 else 13 then then
 \   ?ctrls
    dup to prevch
    ?dup
  until
...
transfer{
  ...
  *key *emit prevch eat-till-cr
  immediate? input-buffer ?echo prompt !prompt aprompt _prompt
...
internals definitions
( Heap Capabilities )
0 mask constant MALLOC_CAP_EXEC
1 mask constant MALLOC_CAP_32BIT
2 mask constant MALLOC_CAP_8BIT
3 mask constant MALLOC_CAP_DMA
10 mask constant MALLOC_CAP_SPIRAM
11 mask constant MALLOC_CAP_INTERNAL
12 mask constant MALLOC_CAP_DEFAULT
13 mask constant MALLOC_CAP_IRAM_8BIT
14 mask constant MALLOC_CAP_RETENTION
15 mask constant MALLOC_CAP_RTCRAM
forth definitions
...
: scope-clear
   ...
   scope-depth ~   scope ~   locals-area locals-here ! ;
...
: ENDOF   cases ++ postpone else ; immediate
...
: setup-saving-base
  here to saving-base  16 cells allot  'cold ~ ;
